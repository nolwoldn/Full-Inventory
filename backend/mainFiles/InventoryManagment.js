...
async function updateItem(req, res) {
  ...
  if (protectedAction && user.userType !== "admin") {
    return res.status(403).json({cause: "Not authorized to change that part"})
  }
  try {
    ...
    console.log(Er);
    return res.status(400).json({cause: `Error ${Er} happened in the backend`})
  }
  return res.status(201).json({ success: true })
}

module.exports = { findCurrentUser, getInventory, manageInventory };
